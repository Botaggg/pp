...
- Forms have antiforgery protection, field validation, a 16 KiB request limit and per-client rate limits. After per-client admission, antiforgery and model validation, login allows 30 attempts per 15 minutes and intake allows 100 submissions per 15 minutes across the application. Honeypot submissions are discarded before consuming this aggregate budget. Unrelated requests and per-client rejections cannot spend another caller's aggregate capacity; authenticated logout has no dependency on either budget. Limits are per application process. The current single-instance deployment is the supported configuration. Multiple instances require a shared limiter or an ingress enforcement service. Enough distinct clients submitting valid requests can still exhaust aggregate capacity; these ceilings protect application resources and are not a complete denial-of-service defense.
